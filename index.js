...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'FN:Orang Tampan😎\n' 
            + 'ORG: Pengembang XBot;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=628819547416:+62881-954-7416\n' 
            + 'END:VCARD' 
blocked = []   
prefix = '#'
limitawal = 30
...
/******** OWNER NUMBER**********/
const ownerNumber = ["628819547416@s.whatsapp.net" , "628829547416@s.whatsapp.net")
/************************************/
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Asikkk beban keluarga masuk🥳@${num.split('@')[0]}\Selamat datang di group *${mdata.subject}* yang betah ya di sini`
				...
				teks = `akhirnya anak pungud keluar bye bye🥳 @${num.split('@')[0]} jasamu selalu di kenang seperti dia yang pernah'
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
          ...
          const checkLimit = (sender) => {
           ...
           return false
            ...
            if (bad.includes(messagesC)) {
                if (!isGroupAdmins) {
                    return reply("JAGA UCAPAN DONG!! EMANG MAU MASUK NERAKA?😠")
				...
				if (messagesC.includes("://chat.whatsapp.com/")){
					...
					var kic = `${sender.split("@")[0]}@s.whatsapp.net`
						reply(`Link Group Terdeteksi maaf anak pungud ${sender.split("@")[0]} anda akan di kick dari group 5detik lagi`)
						setTimeout( () => {
               ...
               case 'help': 
				...
				case 'blocklist': 
					...
					for (let block of blocked) {
						teks += `┣➢ MAMPUS SALAH SENDIRI DOSANYA BANYAK! @${block.split('@')[0]}\n`
					}
         	   ...
         	   case 'creator':
                  ...
                  client.sendMessage(from, 'Tuh nomer owner ku >_<, jangan lupa kenalan ya:v dia baik',MessageType.text, { quoted: mek} )
					break    
				case 'leaderboard':
				case 'lb':
				bo = args[0]
...
• Limit: ${limitawal-updated.limit}`
            ...
            break
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					kapankah = body.slice(1)
					const kapan =['Besok','Lusa','Tadi','4 Hari Lagi','5 Hari Lagi','6 Hari Lagi','1 Minggu Lagi','2 Minggu Lagi','3 Minggu Lagi','1 Bulan Lagi','2 Bulan Lagi','3 Bulan Lagi','4 Bulan Lagi','5 Bulan Lagi','6 Bulan Lagi','1 Tahun Lagi','2 Tahun Lagi','3 Tahun Lagi','4 Tahun Lagi','5 Tahun Lagi','6 Tahun Lagi','1 Abad lagi','3 Hari Lagi','5 Menit Lagi']
           ...
           if (isLimit(sender)) return reply(ind.limitend(pusname))
 			 ...
 			 //  reply('p\n' + util.format({fontPath, inputPath, outputPath, tgl, hari, textnulis}))
				...
				case 'add':
					...
					if (args.length < 1) return reply('Yang mau di add anak setan ya?')
					if (args[0].startsWith('08')) return reply('Gunakan kode negara cuy')
					try {
